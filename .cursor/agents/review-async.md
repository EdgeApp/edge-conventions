...
## Serialize Event Handlers on Shared Resources

When event handlers trigger async operations on shared resources (files, git repos, databases), fire-and-forget patterns cause race conditions. Rapid events create concurrent operations that conflict.

```typescript
// Incorrect - fire-and-forget causes concurrent operations on same resource
feed.on('change', (change) => {
  // Each event immediately starts an operation on the shared git repo
  // Rapid changes cause concurrent git operations → lock errors, conflicts
  handleChange(change).catch(err => log(err))
})

// Correct - serialize operations per resource
const pendingOps = new Map<string, Promise<void>>()

feed.on('change', (change) => {
  const resourceId = change.configId

  // Chain operations: wait for previous, then run new
  const prevOp = pendingOps.get(resourceId) ?? Promise.resolve()
  const newOp = prevOp
    .catch(() => {})  // Ignore previous errors
    .then(() => handleChange(change))
    .catch(err => log(err))

  pendingOps.set(resourceId, newOp)
})
```

**Signs of this bug:**
- Event emitter `.on()` with fire-and-forget async callback (`.catch()` without await)
- The callback operates on filesystem, git repo, or other shared mutable resource
- Multiple events can fire in rapid succession for the same resource

**Rule:** When async event handlers operate on shared resources, serialize operations per resource using a pending operation map or queue.
