...
**Workflow**: Stash changes → Execute plan → Build verification → iOS testing (if affects edge-react-gui or dependencies) → Review loop (max 5 iterations via `revcode`) → Restore stash
...
**Usage**: Provide a plan document path (typically generated by `fixpr` or `revcode`).
