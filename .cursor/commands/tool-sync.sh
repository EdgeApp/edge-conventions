#!/usr/bin/env bash
# tool-sync.sh — Sync Cursor rules, commands, and scripts to OpenCode and Claude Code.
...
sync_opencode() {
  ...
  # Commands: .md → .md + .json
  echo "  Commands:"
  for cmd_md in "$CURSOR_DIR"/commands/*.md; do
    [[ -f "$cmd_md" ]] || continue
    local name
    name=$(basename "$cmd_md" .md)

    sync_file "$cmd_md" "$OPENCODE_DIR/commands/$name.md"

    local json
    json=$(generate_command_json "$cmd_md" "$name")
    sync_content "$json" "$OPENCODE_DIR/commands/$name.json"
  done

  # Scripts: .sh and .js files
  echo "  Scripts:"
  for script in "$CURSOR_DIR"/commands/*.sh "$CURSOR_DIR"/commands/*.js; do
    [[ -f "$script" ]] || continue
    local name
  ...
  if [[ -d "$CURSOR_DIR/skills" ]]; then
    for skill_dir in "$CURSOR_DIR"/skills/*/; do
      ...
      for skill_file in "$skill_dir"*; do
        ...
        sync_file "$skill_file" "$OPENCODE_DIR/skills/$name/$fname"
      done
    done
  fi
  ...
  for oc_cmd in "$OPENCODE_DIR"/commands/*.md; do
    [[ -f "$oc_cmd" ]] || continue
    local name
    name=$(basename "$oc_cmd" .md)
    if [[ ! -f "$CURSOR_DIR/commands/$name.md" ]]; then
      log_action "REMOVE" "$oc_cmd"
      if [[ "$DRY_RUN" == false ]]; then
        rm -f "$oc_cmd" "$OPENCODE_DIR/commands/$name.json"
      fi
      ((removed++)) || true
    fi
  done
...
sync_claude() {
  ...
  # Commands: symlink .md files
  echo "  Commands (symlinks):"
  for cmd_md in "$CURSOR_DIR"/commands/*.md; do
    [[ -f "$cmd_md" ]] || continue
    local name
    name=$(basename "$cmd_md" .md)
    sync_symlink "$cmd_md" "$CLAUDE_DIR/commands/$name.md"
  done
...

  {
    ...
    echo "# Re-generate with: ~/.cursor/commands/tool-sync.sh"
    echo ""
    for mdc in "$CURSOR_DIR"/rules/*.mdc; do
      [[ -f "$mdc" ]] || continue
      echo "@$mdc"
