...
## Merge State Objects on Partial Updates

When updating a nested state object in storage/settings, ensure you merge with the existing state to avoid overwriting concurrent updates from other parts of the app:

```typescript
// Incorrect - overwrites other keys in notifState
return await writeSettings(account, {
  ...settings,
  notifState: newNotifState
})

// Correct - merges with existing keys
return await writeSettings(account, {
  ...settings,
  notifState: {
    ...settings.notifState,
...
This is critical when multiple effects or components may update different keys within the same nested object concurrently. Without merging, one update can silently discard another's changes.
