...
<rule id="use-companion-script">Do NOT call `gh` or `curl` directly. Use `~/.cursor/commands/pr-create.sh` for PR creation (it uses `gh` internally).</rule>
...
**Read `~/.cursor/commands/im.md` now** (use the Read tool — do NOT skip this). Then follow its steps 1-3 (branch setup, pre-change lint check, implementation) using the Asana task context from step 1.
