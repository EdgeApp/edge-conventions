...
<rule id="use-companion-script">Use `~/.cursor/commands/cursor-chat-extract.js` to parse the export. Do NOT parse the raw JSON inline — it is deeply nested and will consume excessive context.</rule>
...
```bash
~/.cursor/commands/cursor-chat-extract.js <export-file> --tools-only
```
...
```bash
Read ~/.cursor/commands/<invokedCommand>.md
```
