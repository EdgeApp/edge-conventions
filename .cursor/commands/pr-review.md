...
<rule id="use-companion-script">Use `~/.cursor/commands/github-pr-review.sh` for all GitHub API operations. Do not use raw `curl`, `gh`, or MCP tools inline.</rule>
...
```bash
~/.cursor/commands/github-pr-review.sh context [--pr <number>] [--owner <owner>] [--repo <repo>]
```
...
```bash
echo '<review-json>' | ~/.cursor/commands/github-pr-review.sh submit \
